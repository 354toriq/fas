            ...
            <div class="modal-body">
                <table class="table table-bordered">
                    <tr>
                        <td>Nama peserta</td>
                        <td>
                            <input class="form-control" type="text" id="nama_peserta">
                        </td>
                    </tr>
                    <tr>
                        <td>TPQ</td>
                            ...
                            <select class="form-control" id='tpq_peserta'>
                                ...
                                <?php $no =1 ?>
                                    <?php foreach($table_tpq as $tpq){ ?>
                                        <option value="<?php echo $tpq->id_tpq;?>">
                                            <?php echo $tpq->nama_tpq;?>
                                        </option>
                                        <?php }?>
                            </select>
                        </td>
...
<div id="modal_sub">
    <?php $no =1 ?>
        <?php foreach($table_peserta as $delete_peserta){ ?>
            <!-- Hapus Data peserta -->
            ...
            <?php } ?>
                ...
                <?php $no =1 ?>
                    <?php foreach($table_peserta as $edit_peserta){ ?>
                                    ...
                                    <div class="modal-body">
                                        <table class="table table-bordered">
                                            <tr>
                                                <td>Nama peserta</td>
                                                <td>
                                                    <input class="form-control" type="text" id="nama_peserta<?php echo $edit_peserta->id_peserta;?>" value="<?php echo $edit_peserta->nama_peserta;?>">
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>TPQ</td>
                                                <td>
                                                    <select class="form-control" id='tpq_peserta<?php echo $edit_peserta->id_peserta;?>'>
                                                        ...
                                                        <?php $no =1 ?>
                                                            <?php foreach($table_tpq as $tpq){ ?>
                                                                <option value="<?php echo $tpq->id_tpq;?>" <?php if($tpq->id_tpq == $edit_peserta->id_tpq) {echo "selected";} else{ echo ""; }?>>
                                                                    <?php echo $tpq->nama_tpq;?>
                                                                </option>
                                                                <?php }?>
                                                    </select>
                                       ...
                                       INNER JOIN tb_lomba b ON a.id_lomba = b.id_lomba WHERE `a`.`id_peserta` = '$edit_peserta->id_peserta' ;" );
                                            $get = $query->result();          
                                                                   $array = []; 
                                                                   
                                                    foreach($get as $each){
                                                        array_push($array,$each->id_lomba);
                                                        ?>
                                                            <input type="checkbox" value="<?=$each->id_lomba?>" name="lomba[]" checked>
                                                            <?=$each->nama_lomba?>
                                                                <br>
                                                                <?php
                                            ...
                                            FROM tb_lomba a
                                            WHERE a.id_lomba NOT IN ('.$implode_array.');' );
                                            $get2 = $query2->result();     
                                                                  
                                                                   
                                                    foreach($get2 as $each){                                                        
                                                        ?>
                                                                    <input type="checkbox" value="<?=$each->id_lomba?>" name="lomba[]">
                                                                    <?=$each->nama_lomba?>
                                                                        <br>
                                                                        <?php
...
<script>
    ...
    function edit_peserta(id_peserta) {
        $.ajax({
            ...
            method: 'post',
            data: 'nama_peserta=' + $('#nama_peserta' + id_peserta).val() +
